...
* Java 17 (für SnapCenter 6.2.1 und höher) oder Java 11 (für SnapCenter 6.2 und früher) muss auf Ihrem Linux- oder AIX-Host installiert sein.
+
** Java von Oracle, OpenJDK oder SAP JDK wird unter Linux unterstützt.
** IBM Java für AIX. Sie können herunterladen von https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX["IBM Semeru Runtimes Downloads"^].
+
...
* Bearbeiten Sie die  `/etc/ssh/sshd_config` Datei, um die Message-Authentifizierungscode-Algorithmen zu konfigurieren: MACs hmac-sha2-256 und MACs hmac-sha2-512.
+
...
Erforderliche Softwarepakete
 a| 
* IBM Java 17 für SnapCenter 6.2.1 und höher oder IBM Java 11 für SnapCenter 6.2 und früher
...
CAUTION: Obwohl es Ihnen erlaubt ist, Anmeldeinformationen nach der Bereitstellung von Hosts und der Installation von Plug-ins zu erstellen, ist es die *beste Vorgehensweise*, Anmeldeinformationen zu erstellen, nachdem Sie SVMs hinzugefügt haben und bevor Sie Hosts bereitstellen und Plug-ins installieren.
...
NOTE: Wenn Sie beim Erstellen von Anmeldeinformationen falsche Angaben machen, wird eine Fehlermeldung angezeigt. Sie müssen *Abbrechen* auswählen und dann erneut versuchen.
...
. Wählen Sie image:../media/filter_icon.gif["Filtersymbol"] und anschließend den Hostnamen und den Datenbanktyp aus, um die Ressourcen zu filtern.
+
Anschließend können Sie image:../media/filter_icon.gif["Filtersymbol"] das Filterfenster schließen.

. Wählen Sie die Datenbank und anschließend *Datenbankeinstellungen* > *Datenbank konfigurieren*.
