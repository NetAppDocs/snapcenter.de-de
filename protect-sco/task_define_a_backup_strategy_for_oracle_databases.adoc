...
Aktuelle Informationen zu unterstützten Oracle-Versionen finden Sie unter https://["NetApp Interoperabilitäts-Matrix-Tool"^].
...
Die Informationen zu den Parametern, die mit dem Befehl und deren Beschreibungen verwendet werden können, können durch Ausführen von get-Help Command_Name abgerufen werden. Alternativ können Sie auch auf die verweisen https://["SnapCenter Software Command Reference Guide"^].
