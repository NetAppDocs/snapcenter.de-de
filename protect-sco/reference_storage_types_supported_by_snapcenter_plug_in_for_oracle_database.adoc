...
Andere Hypervisoren
 a| 
Linux wird unter KVM, Xen, Hyper-V und Hyperscaler-Hypervisoren unterstützt, wenn das Gastbetriebssystem direkten TCP/IP-Zugriff auf die benötigten ONTAP-Speicherressourcen über die folgenden Methoden hat:
...
NOTE: Datastores und Rohgerätedarstellungen werden außerhalb von VMware nicht unterstützt.
