...
NOTE: Für die Dateisystemkonsistenz werden Konsistenzgruppen-Snapshots für die an der Volumengruppe beteiligten LUNs erstellt.
...
NOTE: Wenn Sie der Ressourcengruppe verschiedene Dateisysteme hinzugefügt haben, werden alle Volumes aus verschiedenen Dateisystemen in der Ressourcengruppe in einer Konsistenzgruppe zusammengefasst. Für absturzkonsistente Backups wird standardmäßig ein Snapshot der Konsistenzgruppe erstellt, selbst wenn nur ein Dateisystem oder Volume vorhanden ist.
