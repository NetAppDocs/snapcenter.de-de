...
+
Bei ONTAP 9.12.1 und älteren Versionen übernehmen die aus den SnapLock Vault Snapshot Kopien erstellten Klone die Verfallszeit von SnapLock Vault. Der Storage-Administrator sollte die Klone nach Ablauf der SnapLock-Gültigkeitsdauer manuell bereinigen.
