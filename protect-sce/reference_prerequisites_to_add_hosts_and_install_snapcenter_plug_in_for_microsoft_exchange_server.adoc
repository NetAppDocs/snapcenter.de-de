...
* Sie müssen Microsoft Exchange Server für Einzelplatz- und Datenbankverfügbarkeitsgruppenkonfigurationen verwenden. Informationen zu den unterstützten Versionen von Microsoft Exchange Server finden Sie in der link:https://imt.netapp.com/imt/#search["Interoperabilitätsmatrix-Tool (IMT)"^].
...
* Stellen Sie sicher, dass die folgenden Ports in der Firewall nicht blockiert sind, andernfalls schlägt der Host-Hinzufügen-Vorgang fehl. Um dieses Problem zu beheben, müssen Sie den dynamischen Portbereich konfigurieren. Weitere Informationen finden Sie unter  https://docs.microsoft.com/en-us/troubleshoot/windows-server/networking/configure-rpc-dynamic-port-allocation-with-firewalls["Microsoft-Dokumentation"^]. Informationen zu den unterstützten Versionen von Microsoft Exchange Server finden Sie unter link:https://imt.netapp.com/imt/#search["Interoperabilitätsmatrix-Tool (IMT)"^].
+
