...
* `security login role create -role Role_Name -cmddirname "cluster identity modify" -vserver Cluster_name or cluster_name -access all`
* `security login role create -role Role_Name -cmddirname "cluster identity show" -vserver Cluster_name -access all`
