...
Um einen zusätzlichen Befehl auf dem SnapCenter-Plug-in für PostgreSQL auszuführen, müssen Sie ihn in die Datei aufnehmen `allowed_commands.config` .

`allowed_commands.config` Datei befindet sich im Unterverzeichnis "etc" des SnapCenter Plug-in for PostgreSQL Verzeichnisses.
