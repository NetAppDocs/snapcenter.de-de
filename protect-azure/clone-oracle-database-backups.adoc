...
== Teilen Sie einen Oracle-Datenbankklon auf
...
. Wählen Sie die geklonte Ressource (z. B. Datenbank oder LUN) aus und klicken Sie dann auf *image:../media/clone_icon.gif["Klon-Symbol"]*.
...
. Wählen Sie in der Ansicht Kopien verwalten die Option *Clones* aus den primären Speichersystemen aus.
. Wählen Sie den PDB-Klon (targetCDB:PDBClone) aus, und klicken Sie dann auf *image:../media/clone_icon.gif["Klon-Symbol"]*.
. Überprüfen Sie die geschätzte Größe des zu teilenden Klons und den benötigten Speicherplatz auf dem Aggregat, und klicken Sie dann auf *Start*.
. Überwachen Sie den Fortschritt des Vorgangs, indem Sie auf *Monitor* > *Jobs* klicken.
