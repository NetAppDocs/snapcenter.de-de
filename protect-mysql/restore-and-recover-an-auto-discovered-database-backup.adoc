...
+
Für Linux: _/var/opt/snapcenter/scc/allowed_commands.config_
...
. Wählen Sie auf der Seite Wiederherstellungsumfang die Option *komplette Ressource* aus, um die konfigurierten Datenvolumes der MySQL-Datenbank wiederherzustellen.
...
NOTE: Für Pre- und Post-Befehle für Quiesce-, Snapshot- und Unquiesce-Vorgänge sollten Sie überprüfen, ob die Befehle in der Befehlsliste vorhanden sind, die auf dem Plug-in-Host über den _/opt/snapcenter/snapenter/scc/allowed_commands.config_-Pfad für Linux verfügbar ist.
