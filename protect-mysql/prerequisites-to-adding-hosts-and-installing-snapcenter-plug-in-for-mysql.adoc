...
+
https://imt.netapp.com/matrix/imt.jsp?components=121071;&solution=1259&isHWU&src=IMT["NetApp Interoperabilitäts-Matrix-Tool"]
...
Um einen zusätzlichen Befehl für das SnapCenter-Plug-in für MySQL auszuführen, müssen Sie ihn in die Datei aufnehmen `allowed_commands.config` .

`allowed_commands.config` Die Datei befindet sich im Unterverzeichnis "etc" des SnapCenter Plug-in for MySQL Verzeichnisses.
...
Benutzerdefinierter Pfad: `<Custom_Directory>allowed_commands.config`
