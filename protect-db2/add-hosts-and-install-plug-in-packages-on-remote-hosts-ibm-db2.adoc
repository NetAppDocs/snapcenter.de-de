...
+
Wenn Sie das Kontrollkästchen *Vorabprüfungen überspringen* nicht aktiviert haben, wird der Host überprüft, um festzustellen, ob er die Voraussetzungen für die Installation des Plug-ins erfüllt. Der Festplattenspeicher, der Arbeitsspeicher, die PowerShell-Version, die .NET-Version, der Speicherort (für Windows-Plug-ins) sowie Java 17, Java 11 (nur für SnapCenter 6.2) für Windows- und Linux-Plug-ins werden mit den Mindestanforderungen abgeglichen. Werden die Mindestanforderungen nicht erfüllt, werden entsprechende Fehler- oder Warnmeldungen angezeigt.
