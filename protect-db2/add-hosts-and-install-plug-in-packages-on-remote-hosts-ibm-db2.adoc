...
+
Wenn Sie das Kontrollkästchen „Vorabprüfungen überspringen“ nicht aktiviert haben, wird der Host überprüft, um festzustellen, ob er die Voraussetzungen für die Installation des Plug-ins erfüllt. Dabei werden Festplattenspeicher, Arbeitsspeicher, PowerShell-Version, .NET-Version, Installationsort (für Windows-Plug-ins) sowie Java 17 (für SnapCenter 6.2.1 und höher) bzw. Java 11 (für SnapCenter 6.2 und niedriger) für Windows- und Linux-Plug-ins mit den Mindestanforderungen abgeglichen. Werden die Mindestanforderungen nicht erfüllt, werden entsprechende Fehler- oder Warnmeldungen angezeigt.
